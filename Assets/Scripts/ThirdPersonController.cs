...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private void HandleReload()
        {
            ...
            // 입력이 들어왔을 때
            if (_input.reload && gun != null)
            {
                // Reload 시도, 성공하면 상태 전환
                if (gun.Reload())
                {
                    isReloading = true;
                    if (_hasAnimator)
                ...
                _input.reload = false; // 입력은 항상 리셋
            }
            Debug.Log($"HandleReload: isReloading {isReloading}, gunstate {gun.state}");

        }


        // --------- Animation Event에서 호출 ---------
        // 재장전 Animation Clip의 마지막에 AnimationEvent로 넣어주세요
        public void OnReloadFinished()
        {
            Debug.Log("OnReloadFinished 호출됨");
            isReloading = false;
        }
